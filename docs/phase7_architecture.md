...
| # | 事项 | 优先级 | 说明 |
|---|------|:--:|------|
| 1 | **稳定性测试** | 高 | 跑几轮完整链路，确定单 cookie 每日安全上限（当前 `cookie_daily_max_detail=0` 不限） |
| 2 | **评分失败重试** | 中 | `ai/scorer.py` 中 API 调用失败时自动重试 2-3 次 |
| 3 | **枚举标准化** | 低 | `cleaning/job_cleaner.py` 中公司规模/学历/经验做统一映射 |
| 4 | **企业增强证据质量** | 低 | 知乎摘要截断、关键词主题化、语义匹配 |
| 5 | **Boss/智联接入** | 低 | daemon 已支持 `--platform` 参数，crawler 层待实现 |
| 6 | **WebSocket 实时推送** | 低 | 前端轮询改为实时推送 |
